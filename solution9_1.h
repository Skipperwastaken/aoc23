#ifndef AOC23_SOLUTION9_1_H
#define AOC23_SOLUTION9_1_H
...
int getNextNumber(const std::vector<int> &nums) {
    ...
    if (allZeroes)
        return nums.back();
    return nums.back() + getNextNumber(nextNums);
}
...
void solution() {
    ...
    for(; getline( file, line );) {
        ...
        sum += getNextNumber(stringToNumVec(line));
    }

    std::cout << "sum: " << sum << std::endl;
}

#endif //AOC23_SOLUTION9_1_H
